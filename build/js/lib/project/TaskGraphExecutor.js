var EventEmitter, Multimap, TaskGraphExecutor, _, log, rek,
...
TaskGraphExecutor = (function(superClass) {
  ...
  TaskGraphExecutor.prototype.add = function(nodes) {
    var dependsOnTasks, filtered, j, k, len, len1, node, queue, results, sorted, t, targetNode, task, visiting;
    queue = [];
    ...
    while (queue.length) {
      ...
      if (!visiting.has(node)) {
        ...
        for (k = 0, len1 = dependsOnTasks.length; k < len1; k++) {
          t = dependsOnTasks[k];
